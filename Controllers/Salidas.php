<?php
    class Salidas extends Controller{
        ...
        public function registrarSalida(){
            ...
            $motivo = $_POST['motivo'];
            if (empty($id_documento) || empty($n_documento) || empty($motivo)) {
                ...
                if($data == 'ok'){
                    ...
                    foreach($detalle as $row){
                        ...
                        $total_salida=$this->model->getProductos($id_pro);
                        $salida=$total_salida['total_salida'] + $sub_total;
                        $this->model->actualizarSalida($salida, $id_pro);
        ...
        public function  listar_historial(){
           ...
           for ($i=0; $i<count($data); $i++){
                if($data[$i]['estado'] ==1){
                    ...
                    $data[$i]['acciones']='<div>
                    <button class="btn btn-warning mb-2" type="button" onclick="btnAnularS('.$data[$i]['id'].')"><i class="fas fa-ban"></i></button>
        ...
        public function anularSalida($id_salida){
            $data = $this->model->getAnularSalida($id_salida);
            $anular = $this->model->getAnular($id_salida);
            foreach ($data as $row) {
                ...
                $total_salida=$this->model->getProductos($row['id_producto']);
                $salida=$total_salida['total_salida'] - $row['sub_total'];
                $this->model->actualizarSalida($salida, $row['id_producto']);
            ...
            if ($anular == 'ok') {
                $msg = array('msg' => 'Salida Anulada', 'icono' => 'success');
            }else {
                $msg = array('msg' => 'Error al anular la Salida', 'icono' => 'error');
            }
            echo json_encode($msg, JSON_UNESCAPED_UNICODE);
            die();
        }
